...
> 🤖 **CLI 硬约束在 [`CLAUDE.md`](./CLAUDE.md)**（自动加载），指令码完整定义在 [`指令码.md`](./指令码.md)。
> **指令码 #1「进度定位」**：说出那句触发语 ⇒ CLI 先 `git pull` 三个仓库，再读断点 + 排期两篇并对照实际代码交叉核对，最后输出「位置 / 文档与代码是否一致 / 下一步」。写在仓库里而非 CLI 本地记忆，**是为了跨机器生效**。
...
├─ README.md                    ← 本文
│
├─ CLAUDE.md                 ★ CLI 硬约束（自动加载，跨机器生效）
├─ 指令码.md                 ★ 触发语 → 固定动作的完整定义
│
...
│
├─ process-front/                前端文档
│   ├─ 页面思考法/           ★ 写页面的思考顺序：① 布局 → ② 组件 → ③ 样式（方法论，跨页面复用）
...
| **方法论** | **怎么想**（不是「是什么」），跨页面跨项目复用 | [`process-front/页面思考法/`](./process-front/页面思考法/00-总纲.md) | 沉淀出新方法时 |

> **写任何新前端页面前，先读 [`process-front/页面思考法/00-总纲.md`](./process-front/页面思考法/00-总纲.md)**，并按 ① 布局 → ② 组件 → ③ 样式 的顺序推进。约束条款在 [`讲解规则.md`](./讲解规则.md) 第七节。
